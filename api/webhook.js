...
async function sendMsg(text, ik = null, kb = null, targetChatId = SUPER_ADMIN) {
    const p = { chat_id: targetChatId, text, parse_mode: 'HTML' };
    if (ik) p.reply_markup = { inline_keyboard: ik };
    else if (kb) p.reply_markup = { keyboard: kb, resize_keyboard: true };
    else if (isAdmin(targetChatId)) p.reply_markup = { keyboard: mainKb(targetChatId), resize_keyboard: true };
    await tg('sendMessage', p);
}

async function broadcast(message) {
    const { data: users } = await supabase.from('users').select('telegram_id');
    const ids = users?.map(u => u.telegram_id).filter(id => id && !isAdmin(id)) || [];
    let count = 0;
    for (const id of ids) {
        try {
            await tg('sendMessage', { chat_id: id, text: `📢 <b>إعلان جديد:</b>\n\n${message}`, parse_mode: 'HTML' });
            count++;
        } catch (e) {
    ...
    try {
        if (update.message) {
            ...
            if (isAdmin(chatId)) {
                ...
                // Handle non-admin users (students)
                await handleStudentMessage(update.message);
            }
        } else if (update.callback_query && isAdmin(update.callback_query.message.chat.id)) {
...
يجب على الطالب إرسال "اسم المستخدم" الخاص به لهذا البوت ليتم ربط حسابه واستلام الإعلانات.`;
    ...
    if (state.action) {
        ...
        if (action === 'broadcast_mode') {
            ...
            const count = await broadcast(input);
            return sendMsg(`✅ تم إرسال الإعلان إلى <b>${count}</b> طالب.`);
        }
        ...
        if (action === 'add_admin_id') {
            ...
            if (admins.includes(newId)) return sendMsg("⚠️ هذا المستخدم أدمن بالفعل.");
            await saveAdmins([...admins, newId]);
        ...
        if (action === 'site_ann_url') {
            ...
            return sendMsg("✅ تم تحديث إعلان الموقع بنجاح!");
        }
...
async function handleCallback(data) {
    ...
    if (data === 'list_admins') {
        ...
        msg += `👑 <b>أنت (Super Admin)</b> - كل الصلاحيات\n\n`;
        const ik = [];
        ids.forEach(id => {
            msg += `• <code>${id}</code>\n`;
            ik.push([{ text: `⚙️ صلاحيات ${id}`, callback_data: `edit_perms:${id}` }, { text: `🗑️ حذف`, callback_data: `conf_del_admin:${id}` }]);
        });
    ...
    if (data.startsWith('edit_perms:')) {
        ...
        let msg = `⚙️ <b>صلاحيات الأدمن:</b> <code>${id}</code>\nاختر الصلاحية لتفعيلها أو تعطيلها:\n`;
        const ik = [];
        Object.keys(PERMISSIONS_MAP).forEach(pk => {
            const has = perms.includes(pk);
            msg += `${has ? '✅' : '❌'} ${PERMISSIONS_MAP[pk]}\n`;
            ik.push([{ text: `${has ? '🔴 تعطيل' : '🟢 تفعيل'} ${PERMISSIONS_MAP[pk]}`, callback_data: `tog_perm:${id}:${pk}` }]);
        });
