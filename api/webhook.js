...
يجب على الطالب إرسال "اسم المستخدم" الخاص به لهذا البوت ليتم ربط حسابه واستلام الإعلانات.`;
    ...
    if (input.startsWith('👤 ')) {
        // تنظيف الاسم المستخرج من الزر ليدعم الأسماء التي تحتوي على مسافات
        let uname = input.substring(2); // تخطي "👤 "
        // إذا كان هناك رمز حالة (أيقونة) نقوم بإزالتها أيضاً
        const emojiMatch = uname.match(/^[🟢🔴❓🚫🟡✅]\s*/u);
        if (emojiMatch) {
            uname = uname.substring(emojiMatch[0].length);
        }
        uname = uname.trim();
