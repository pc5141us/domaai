...
يجب على الطالب إرسال "اسم المستخدم" الخاص به لهذا البوت ليتم ربط حسابه واستلام الإعلانات.`;
    ...
    if (input.startsWith('👤 ')) {
        // تحسين استخراج اسم المستخدم من الزر ليشمل كافة الحالات (نشط، منتهي، معلق)
        let uname = input.replace(/^👤\s*[🟢🔴❓🚫🟡✅]\s*/u, '').trim();
        // إذا فشل الريجكس في التنظيف (مثل حالة وجود رموز أخرى)، نستخدم طريقة التقسيم
        if (uname.startsWith('👤') || uname.includes(' ')) {
            const parts = input.trim().split(/\s+/);
            uname = parts[parts.length - 1]; // اسم المستخدم يكون دائماً آخر كلمة
        }
