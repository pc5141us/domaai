...
async function handleMessage(text, chatId = null) {
    ...
    if (input === '🧨 تصفير جميع الأدمنية') {
        ...
        return sendMsg("💥 <b>تحذير نهائي:</b> هل تريد فعلاً حذف جميع الأدمنية؟", null, [[{ text: '⚠️ نعم، قم بالتصفير' }], [{ text: '🔙 رجوع' }]], chatId);
    }
    
    if (input === '⚠️ نعم، قم بالتصفير') {
        if (chatId.toString() !== SUPER_ADMIN.toString()) return;
        const perms = await fetchAdmins();
        const admins = Object.keys(perms);
        for (const id of admins) {
            if (id !== SUPER_ADMIN.toString()) delete perms[id];
        }
        await saveAdmins(perms);
        await clearState(chatId);
        return sendMsg("✅ تم تصفير جميع الأدمنية بنجاح.", null, mainKb(chatId), chatId);
    }
    ...
    if (input.startsWith('✅ ') || input.startsWith('❌ ')) {
        const state = await getState(chatId);
        if (state.action === 'editing_perms' && state.targetId) {
            const label = input.slice(2);
            const pk = Object.keys(PERMISSIONS_MAP).find(k => PERMISSIONS_MAP[k] === label);
            if (pk) return handleCallback(`tog_perm:${state.targetId}:${pk}`, chatId);
        }
    }
    ...
    if (input === '🔙 رجوع') {     const state = await getState(chatId);
        ...
        if (action === 'conf_del_admin') {
            const state = await getState(chatId);
            return handleCallback(`edit_perms:${state.targetId}`, chatId);
        }
...
async function handleCallback(data, chatId = null) {
    ...
    if (data.startsWith('del_admin:')) {
        ...
        return handleCallback('list_admins', cid);
    }
