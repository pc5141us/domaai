...
async function handleMessage(text, chatId = null) {
    ...
    if (input.includes('تفعيل') || input.includes('تعطيل')) {
        const state = await getState(chatId);
        if (state.targetId) {
            // البحث عن الصلاحية بناءً على الكلمات الفريدة في المسميات
            const pk = Object.keys(PERMISSIONS_MAP).find(k => {
                const label = PERMISSIONS_MAP[k].replace(/[^\w\s\u0600-\u06FF]/g, '').trim();
                return input.includes(label) || input.includes(PERMISSIONS_MAP[k]);
            });
            if (pk) return handleCallback(`tog_perm:${state.targetId}:${pk}`, chatId);
        } else {
            return sendMsg("⚠️ انتهت الجلسة أو لم يتم تحديد أدمن. يرجى العودة وقائمة الأدمنز والاختيار مجدداً.", null, [[{ text: '🔙 رجوع' }]], chatId);
        }
    }
...
async function handleCallback(data, chatId = null) {
    ...
    if (data.startsWith('tog_perm:')) {
        ...
        if (perms.includes(pk)) {
            CACHED_ADMINS_PERMS[id] = perms.filter(x => x !== pk);
        } else {
            CACHED_ADMINS_PERMS[id] = [...perms, pk];
        }
        await saveAdmins(CACHED_ADMINS_PERMS);
        const status = CACHED_ADMINS_PERMS[id].includes(pk) ? 'تم تفعيل' : 'تم تعطيل';
        await tg('sendMessage', { chat_id: cid, text: `🔹 <b>تحديث:</b> ${status} صلاحية (<b>${PERMISSIONS_MAP[pk]}</b>) للأدمن بنجاح.` });
