...
async function saveState(state) {
    ...
    if (data) {
        await supabase.from('coupons').update({ used_by: jsonStr }).eq('code', 'BOT_STATE_ADMIN');
    } else {
        await supabase.from('coupons').insert([{ code: 'BOT_STATE_ADMIN', duration: 1, type: 'days', used_by: jsonStr, created_at: new Date().toISOString() }]);
    }
}
...
async function handleMessage(text) {
    ...
    if (input.startsWith('👤 ')) {
        ...
        uname = uname.replace(/^[🟢🔴🟡✅]\s*/u, '').trim();
        const { data: users } = await supabase.from('users').select('*').eq('username', uname);
        if (users && users.length) return handleCallback(`student_info:${users[0].id}`);
        return sendMsg(`❌ الطالب غير موجود: ${uname}`);
    }
    if (input.startsWith('📖 ')) {
        const title = input.substring(input.indexOf(' ') + 1).trim();
        const { data: lessons } = await supabase.from('lessons').select('*');
        const f = lessons?.find(l => l.title.trim() === title);
        if (f) return handleCallback(`lesson_info:${f.id}`);
        return sendMsg(`❌ الدرس غير موجود: ${title}`);
    }
    const cmatch = input.match(/^[✅🟢]\s*([a-zA-Z0-9]+)/);
    ...
    if (state.action) {
        ...
        if (action === 'search_lesson') {
            ...
            const q = input.toLowerCase();
            const { data: lessons } = await supabase.from('lessons').select('*');
            const res = (lessons || []).filter(l => l.title.toLowerCase().includes(q) || (l.description && l.description.toLowerCase().includes(q)));
...
async function sendStudentsList(page) {
    ...
    sendMsg(`👥 <b>قائمة الطلاب (صفحة ${page + 1}/${totalPages}):</b>\nاختر من الأسفل:`, null, kb);
}
...
async function sendLessonsList(page) {
    ...
    sendMsg(`📚 <b>قائمة الدروس (صفحة ${page + 1}/${totalPages}):</b>\nاختر من الأسفل:`, null, kb);
}
...
async function handleCallback(data) {
    ...
    if (data.startsWith('copy_coupon:')) { sendMsg("اضغط للنسخ:"); return sendMsg(`<code>${data.split(':')[1]}</code>`); }
    ...
    if (data === 'show_codes') {
        ...
        kb.push([{ text: '🔙 رجوع' }]);
        return sendMsg(`🏷️ <b>أحدث الأكواد:</b>`, kb);
    }
