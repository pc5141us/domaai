...
async function handleCallback(data) {
    ...
    if (data === 'list_admins') {
        ...
        // جلب أسماء المستخدمين من قاعدة البيانات بناءً على Telegram ID
        ...
        const ik = [];
        ids.forEach(id => {
            const name = nameMap[id] || 'مجهول (لم يربط حسابه)';
            msg += `• 👤 <b>${name}</b> (<code>${id}</code>)\n`;
            ik.push([{ text: `⚙️ صلاحيات ${name.substring(0, 10)}`, callback_data: `edit_perms:${id}` }, { text: `🗑️ حذف`, callback_data: `conf_del_admin:${id}` }]);
        });
