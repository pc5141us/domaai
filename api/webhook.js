...
توليد أكواد تفعيل (من ساعة إلى سنة) لإهدائها للطلاب أو بيعها.`;
    ...
    if (input.includes('👤')) {
        // استخراج اسم المستخدم من نص الزر بشكل مرن وآمن
        // نقوم بقص كل شيء قبل أول مسافة بعد أيقونة الطالب
        const parts = input.trim().split(/\s+/);
        // نتجاوز أول خانتين (👤 والرمز الملون) ونأخذ باقي النص كاسم مستخدم
        let uname = parts.slice(2).join(' ');
        // إذا لم يكن هناك رمز ملون (في البحث مثلاً)، نأخذ ما بعد 👤
        if (!uname || uname.length < 2) uname = parts.slice(1).join(' ');
