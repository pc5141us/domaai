...
async function getBotConfig() {
    try {
        ...
        return data && data.password ? JSON.parse(data.password) : { admins: {}, announcement: {} };
    } catch (e) { return { admins: {}, announcement: {} }; }
}
...
async function handleMessage(text, chatId = null) {
    ...
    if (input === '➕ إضافة أدمن جديد') {
        return handleCallback('add_admin_start', chatId);
    }
    if (input === '📋 التحكم في الصلاحيات') {
        return handleCallback('list_admins', chatId);
    }
    if (input === '🔙 رجوع') {
        const state = await getState(chatId);
        // إذا كان هناك ID مستهدف، نعود لقائمة الأدمنز
        if (state.targetId || state.action === 'editing_perms') {
            await clearState(chatId);
            return handleCallback('list_admins', chatId);
        }
        // عدا ذلك نعود لإعدادات الإدارة الأساسية
        await clearState(chatId);
...
يجب على الطالب إرسال "اسم المستخدم" الخاص به لهذا البوت ليتم ربط حسابه واستلام الإعلانات.`;
        ...
        return sendMsg(helpMsg, null, null, chatId);
    }
    ...
    if (cmatch) return handleCallback(`coupon_info:${cmatch[1]}`, chatId);
...
async function handleCallback(data, chatId = null) {
    ...
    if (data === 'list_expired') {
        const { data: users } = await supabase.from('users').select('*');
        const now = Date.now();
        const expired = (users || []).filter(u => u.role !== 'admin' && ['active', 'pending'].includes(u.status) && u.expiry_date && new Date(u.expiry_date) <= now);
    ...
    // 3. إدارة الطلاب
    if (data === 'add_student') { await saveState(cid, { action: 'add_user_name' }); return sendMsg("👤 أرسل اسم الطالب الجديد:"); }
    if (data === 'search_student') { await saveState(cid, { action: 'search_student' }); return sendMsg("🔍 أرسل اسم الطالب للبحث عنه:"); }
    ...
    // 4. إدارة الدروس
    if (data === 'add_lesson') { await saveState(cid, { action: 'add_lesson_title' }); return sendMsg("📚 أرسل عنوان الدرس الجديد:"); }
    if (data === 'search_lesson') { await saveState(cid, { action: 'search_lesson' }); return sendMsg("🔍 أرسل عنوان الدرس للبحث عنه:"); }
    ...
    if (data === 'show_codes') {
        ...
        const kb = filtered.map(c => [{ text: `🎫 ${c.code} (${c.duration}${c.type === 'hours' ? 'س' : 'ي'})` }]);
        kb.push([{ text: '🔙 العودة للقائمة الرئيسية' }]);
        return sendMsg(`🏷️ <b>أحدث 20 كود:</b>`, null, kb, cid);
    }
    ...
    if (data === 'cancel') { await clearState(cid); return sendMsg("تم الإلغاء والعودة للقائمة الرئيسية."); }
}

async function sendStudentsList(page, chatId = null) {
    const cid = chatId || SUPER_ADMIN;
    const { data: users } = await supabase.from('users').select('*');
    // فلترة المديرين وأيضاً سجلات النظام التي تبدأ بـ DOMA_AI_ لضمان قائمة نظيفة
    const stds = (users || []).filter(u => u.role !== 'admin' && !u.username.startsWith('DOMA_AI_')).sort((a, b) => b.id - a.id);
    const pageSize = 10;
    ...
    for (let i = 0; i < slice.length; i += 2) {
        ...
        for (let j = i; j < Math.min(i + 2, slice.length); j++) {
            row.push({ text: `👤 ${slice[j].status === 'active' ? '🟢' : '🔴'} ${slice[j].username}` });
        }
        kb.push(row);
    }
    const nav = [];
