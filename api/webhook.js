...
توليد أكواد تفعيل (من ساعة إلى سنة) لإهدائها للطلاب أو بيعها.`;
    ...
    if (input.includes('👤')) {
        ...
        // التمييز بين حالتين: (👤 أيقونة يوزر) أو (👤 يوزر فقط)
        // إذا كان هناك أكثر من جزئين وكان الجزء الثاني طوله قصير (أيقونة)، نتجاوزه
        let uname = (parts.length >= 3 && parts[1].length <= 4) ? parts.slice(2).join(' ') : parts.slice(1).join(' ');
        uname = uname.trim();
