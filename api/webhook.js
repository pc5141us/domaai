...
async function sendMsg(text, ik = null, kb = null, targetChatId = ADMIN_ID) {
    ...
    else if (targetChatId == ADMIN_ID) p.reply_markup = { keyboard: mainKb, resize_keyboard: true };
    await tg('sendMessage', p);
}

async function broadcast(message) {
    const { data: users } = await supabase.from('users').select('telegram_id');
    const ids = users?.map(u => u.telegram_id).filter(id => id && id != ADMIN_ID) || [];
    let count = 0;
    for (const id of ids) {
        try {
