...
async function handleCallback(data) {
    ...
    if (data === 'list_admins') {
        ...
        // جلب الأسماء من قاعدة البيانات أولاً
        ...
        for (const id of ids) {
            ...
            // إذا لم يوجد اسم في القاعدة، نحاول جلبه من تيليجرام مباشرة
            if (!name) {
                try {
                    const res = await fetch(`https://api.telegram.org/bot${BOT_TOKEN}/getChat?chat_id=${id}`).then(r => r.json());
                    if (res.ok) {
            ...
            ik.push([{ text: `⚙️ صلاحيات ${name.substring(0, 15)}`, callback_data: `edit_perms:${id}` }, { text: `🗑️ حذف`, callback_data: `conf_del_admin:${id}` }]);
        }
    ...
    if (data.startsWith('edit_perms:')) {
        ...
        let msg = `⚙️ <b>صلاحيات الأدمن:</b> ${name}\nاختر الصلاحية لتغيير حالتها:`;
        const ik = [];
