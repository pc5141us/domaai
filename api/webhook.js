...
async function getState() {
    const { data } = await supabase.from('lessons').select('description').eq('title', 'BOT_STATE_ADMIN').single();
    if (data && data.description) {
        try { return JSON.parse(data.description); } catch (e) { return {}; }
    }
    return {};
}
...
async function saveState(state) {
    try {
        ...
        const { data } = await supabase.from('lessons').select('id').eq('title', 'BOT_STATE_ADMIN').single();
        if (data) {
            const { error } = await supabase.from('lessons').update({ description: jsonStr }).eq('title', 'BOT_STATE_ADMIN');
            if (error) await sendMsg(`⚠️ Error updating state: ${error.message}`);
        } else {
            const { error } = await supabase.from('lessons').insert([{ title: 'BOT_STATE_ADMIN', description: jsonStr, url: '-', created_at: new Date().toISOString() }]);
...
async function handleMessage(text) {
    ...
    if (state.action) {
        ...
        if (action === 'search_lesson') {
            ...
            const res = (lessons || []).filter(l => l.title !== 'BOT_STATE_ADMIN' && ((l.title || '').toLowerCase().includes(q) || (l.description || '').toLowerCase().includes(q)));
...
async function sendLessonsList(page) {
    const { data: lessons } = await supabase.from('lessons').select('*');
    const ls = (lessons || []).filter(l => l.title !== 'BOT_STATE_ADMIN').sort((a, b) => b.id - a.id);
    const pageSize = 10;
