...
async function handleMessage(text, chatId = null) {
    ...
    if (input.includes('✅') || input.includes('❌')) {
        ...
        if (state.targetId) {
            ...
            return sendMsg("⚠️ الجلسة منتهية، يرجى إعادة الدخول لملف الأدمن من قائمة الأدمنز.");
        }
    }
...
async function updateSiteAnnouncement(obj) {
    const { data } = await supabase.from('users').select('id').eq('username', 'SITE_ANNOUNCEMENT').single();
    if (data) await supabase.from('users').update({ password: JSON.stringify(obj) }).eq('id', data.id);
    else await supabase.from('users').insert([{ username: 'SITE_ANNOUNCEMENT', password: JSON.stringify(obj), role: 'admin' }]);
}
