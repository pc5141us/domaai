...
توليد أكواد تفعيل (من ساعة إلى سنة) لإهدائها للطلاب أو بيعها.`;
    ...
    if (input.includes('👤')) {
        // محاولة استخراج الاسم بذكاء لتجنب مشاكل الأيقونات والمسافات
        let uname = input.substring(input.indexOf('👤') + 1).trim();
        // إذا كان هناك رمز حالة (مثل 🔴) نقوم بقصه هو والمسافة التي تتبعه
        const cleanParts = uname.split(/\s+/);
        if (cleanParts.length >= 2 && cleanParts[0].length <= 5) {
            uname = cleanParts.slice(1).join(' '); // ما بعد الرمز الملون
        } else {
            uname = cleanParts.join(' '); // في حال عدم وجود رمز ملون
        }
        uname = uname.trim();
        
        // استخدام ilike للبحث ليكون مرناً مع حالة الأحرف
        const { data: user } = await supabase.from('users').select('id').ilike('username', uname).single();
    ...
    if (!state.action) {
        const { data: directU } = await supabase.from('users').select('id').eq('username', input).single();
    ...
    if (state.action) {
        ...
        if (action === 'add_admin_id') {
            ...
            await sendMsg(`✅ تمت إضافة <b>${input}</b> كمسؤول جديد بنجاح.`, null, null, chatId);
            return await sendAdminDashboard(input);
        }
