...
توليد أكواد تفعيل (من ساعة إلى سنة) لإهدائها للطلاب أو بيعها.`;
    ...
    if (input.includes('👤')) {
        // استخراج اسم المستخدم بدقة عن طريق حذف الأيقونات والمسافات البادئة بشكل ذكي
        // نقوم بحذف أي رموز هيروغليفية أو تعبيرية في بداية النص حتى نصل لأول حرف أو رقم
        let uname = input.replace(/^[^\p{L}\p{N}]+/u, '').trim();
        
        // التحقق من وجود "👤" داخل الاسم المنظف (حالة نادرة)
        if (uname.includes('👤')) uname = uname.split(' ').pop();
